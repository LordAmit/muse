...
public class TaintSinkOperatorTest {
	...
	public void init() {
		try {
			...
			System.err.println(
					String.format("ERROR PROCESSING \"%s\": %s", "src/test/java/input/sample_multilevelclass.txt", e.getMessage()));
			return;
		}
	...
	public void insert_sink_declaration_nodeChange() throws ClassNotFoundException {
		...
		taintSinkOperator = new TaintSinkOperator(rewriter, nodeChanges,"src/test/java/input/sample_helloWorld.txt");
