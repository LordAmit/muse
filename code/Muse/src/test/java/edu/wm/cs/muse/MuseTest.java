...
public class MuseTest {
	...
	// the expected output.
	File output = new File("src/test/java/output/output.txt");
	...
	public void reset() throws FileNotFoundException {
		...
		FileReader fr = new FileReader("src/test/java/output/output_reset.txt");
		BufferedReader br = new BufferedReader(fr);
		
		try {
			FileWriter fw = new FileWriter("src/test/java/output/output.txt", true);
			String s;
	...
	private void scope_multi_reset() throws FileNotFoundException {
		FileReader fr = new FileReader("src/test/java/output/scope_multi_reset.txt");
		BufferedReader br = new BufferedReader(fr);
		
		try {
			FileWriter fw = new FileWriter("src/test/java/output/output.txt", false);
			String s;
	...
	public void reachability_operation_on_hello_world() throws Exception {
		...
		try {
			...
			Assert.assertEquals(true, FileUtility.testFileEquality(expectedOutput, processedOutput));
	...
	private void prepare_try_test_files() throws FileNotFoundException, IOException {
		...
		//output = new File("src/test/java/output/output.txt");
	
		content = FileUtility.readSourceFile("src/test/java/input/sample_helloWorld.txt").toString();
		expectedOutput = new File("src/test/java/output/sample_hello_world_try_catch_sink.txt");
	...
	private void prepare_try_multi_test_files() throws FileNotFoundException, IOException {
		...
		content = FileUtility.readSourceFile("src/test/java/input/sample_multilevelclass.txt").toString();
		expectedOutput = new File("src/test/java/output/sample_try_catch_multilevel_class_sink.txt");
	...
	private void prepare_test_files(OperatorType operator, int test) throws FileNotFoundException, IOException {
		...
		case TAINTSINK:
			...
			// the code base
			content = FileUtility.readSourceFile("src/test/java/input/sample_helloWorld.txt").toString();
			expectedOutput = new File("src/test/java/output/sample_hello_world_sink.txt");
			break;

		case REACHABILITY:
			if (test == 1) {
				content = FileUtility.readSourceFile("src/test/java/input/sample_helloWorld.txt").toString();
				expectedOutput = new File("src/test/java/output/sample_hello_world_reachability.txt");
			}
			else if (test == 2) {
				content = FileUtility.readSourceFile("src/test/java/input/sample_helloWorld.txt").toString();
				expectedOutput = new File("src/test/java/output/sample_hello_world_try_catch_reachability.txt");
			}
			break;
		...
		case COMPLEXREACHABILITY:
			if (test ==1) {
				content = FileUtility.readSourceFile("src/test/java/input/sample_helloWorld.txt").toString();
				expectedOutput = new File("src/test/java/output/sample_hello_world_complex_reachability.txt");
			}
			else if (test == 2) {
				content = FileUtility.readSourceFile("src/test/java/input/sample_helloWorld.txt").toString();
				expectedOutput = new File("src/test/java/output/sample_hello_world_try_catch_complex_reachability.txt");
			}
			break;
			
		case TAINTSOURCE:
			content = FileUtility.readSourceFile("src/test/java/input/sample_helloWorld.txt").toString();
			expectedOutput = new File("src/test/java/output/sample_hello_world_source.txt");
			break;

		case SCOPESOURCE:
			if (test == 1) {
				content = FileUtility.readSourceFile("src/test/java/input/sample_helloWorld.txt").toString();
				expectedOutput = new File("src/test/java/output/sample_hello_world_taint.txt");
			}
			else if (test == 2) {
				content = FileUtility.readSourceFile("src/test/java/input/sample_multilevelclass.txt").toString();
				expectedOutput = new File("src/test/java/output/sample_multilevelclass_taint.txt");
			}
			break;
			
		case SCOPESINK:
			content = FileUtility.readSourceFile("src/test/java/input/sample_multilevelclass.txt").toString();
			expectedOutput = new File("src/test/java/output/sample_multilevelclass_taint_sink.txt");
			break;
			
		case IVH:
			if (test == 1) {
				content = FileUtility.readSourceFile("src/test/java/input/sample_class_extended.txt").toString();
				expectedOutput = new File("src/test/java/output/sample_class_extended_output.txt");
			}
			else if (test == 2) {
				content = FileUtility.readSourceFile("src/test/java/input/sample_class_extended_from_B.txt").toString();
				expectedOutput = new File("src/test/java/output/sample_class_extended_from_B_output.txt");
			}
			else if (test == 3) {
				content = FileUtility.readSourceFile("src/test/java/input/sample_class_extended_twice.txt").toString();
				expectedOutput = new File("src/test/java/output/sample_class_extended_twice_output.txt");
			}
			break;
		}
