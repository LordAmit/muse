...
public class TaintSourceOperatorTest {
	...
	public void setUp() {
		try {
			source = FileUtility.readSourceFile("src/test/java/input/sample_helloWorld.txt").toString();
		}
		catch (IOException e) {
			System.err.println(String.format("ERROR PROCESSING \"%s\": %s", "src/test/java/input/sample_helloWorld.txt", e.getMessage()));
			return;
		}
	...
	public void insert_declaration_nodeChange() {	
		...
		taintSourceOperator = new TaintSourceOperator(rewriter, nodeChanges,"src/test/java/input/sample_helloWorld.txt", true);
