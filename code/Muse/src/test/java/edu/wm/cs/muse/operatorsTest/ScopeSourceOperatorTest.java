...
public class ScopeSourceOperatorTest {
	...
	public void setUp() {
		try {
			source = FileUtility.readSourceFile("src/test/java/input/sample_multilevelclass.txt").toString();
		}
		catch (IOException e) {
			System.err.println(String.format("ERROR PROCESSING \"%s\": %s", "src/test/java/input/sample_multilevelclass.txt", e.getMessage()));
			return;
		}
	...
	public void insertion_at_null_declaration_node() {
		...
		scopeSourceOperator = new ScopeSourceOperator(rewriter, nodeChanges,"src/test/java/input/sample_multilevelclass.txt", true);
